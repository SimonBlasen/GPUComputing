...
bool CAssignmentBase::InitCLContext()
{
	...
	cl_ulong globalMemorySize;
	clGetDeviceInfo(m_CLDevice, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &globalMemorySize, NULL);
	cout << "Global memory size: " << (int)(globalMemorySize / (1024 * 1024 * 1024)) << "GB (" << globalMemorySize << " Byte)" << std::endl;

	cl_uint maxWorkDims;
	clGetDeviceInfo(m_CLDevice, CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_uint), (void*)&maxWorkDims, NULL);
	std::cout << "Work Dimensions: " << maxWorkDims << std::endl;

	std::vector<size_t> maxWorkItemsPerDim;
	maxWorkItemsPerDim.resize(maxWorkDims);
	clGetDeviceInfo(m_CLDevice, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(size_t) * maxWorkDims, (void*)&maxWorkItemsPerDim[0], NULL);
	std::cout << "Max Work-Item sizes: (";
	for (int i = 0; i < maxWorkItemsPerDim.size(); i++)
	{
		cout << maxWorkItemsPerDim[i];
		if (i < maxWorkItemsPerDim.size() - 1)
		{
			cout << ", ";
		}
	}
